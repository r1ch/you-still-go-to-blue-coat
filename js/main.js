		...
		<div>
			<div class="jumbotron" v-if = "attendee">
				<div class="container">
					...
					<br><br>
					<p class="lead" v-if = "attendee.reporter"><i>... according to {{attendee.reporter}}</i></p>
...
Vue.component('ysgtb-d3', {
	...
	methods: {
		draw() {		
			...
			let lineLabels = this.svg.selectAll('textPath.lineLabel')
				.data(timeSeries)
				.join(enter=>enter
					.append('text')
					...
					.call(
				      ...
				      update=>update.text((d,i)=>i>0?"":`${d[0].name} : growing in grace`)
				 )
